...
export function SidebarItem({ icon, text, active, alert, href, method, children, as, categoryIcon }) {
    ...
    return (
        ...
        transition-colors group ${active ? "bg-gradient-to-tr from-borderHover-900 to-borderHover-100 text-white" :
            "hover:bg-green-50 text-primary  "}`}>
            {renderLink()}
            {alert && (
            ...
            {children && (
                <ul className={`ml-4 ${isOpen ? "block" : "hidden"} ${expanded ? "w-40  " : "max-content "}`} >
                    {children}
                </ul>
            )}
        </li>
